...
class LogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    broadLeafPath.cubicTo(
      -leafWidth * 0.68, -leafLen * 0.35,
      -leafWidth * 0.58, -leafLen * 0.78,
      0, -leafLen,
    );
    // Right broad curve
    broadLeafPath.cubicTo(
    ...
    final Paint broadLeafFill = Paint()
      ..shader = const LinearGradient(
        ...
        stops: [0.0, 0.55, 1.0],
      ).createShader(Rect.fromLTWH(-leafWidth, -leafLen, leafWidth * 2, leafLen))
      ..style = PaintingStyle.fill;
    ...
    stemPath.quadraticBezierTo(stemControl.dx, stemControl.dy, stemEnd.dx, stemEnd.dy);
    ...
    void drawLeaf(Offset pos, double angleRad, double leafSize) {
      ...
      leafPath.moveTo(0, 0);
      leafPath.quadraticBezierTo(-leafSize * 0.25, -leafSize * 0.4, 0, -leafSize);
      leafPath.quadraticBezierTo(leafSize * 0.25, -leafSize * 0.4, 0, 0);

      leafFillPaint.shader = const LinearGradient(
      ...
      final Paint veinPaint = Paint()
        ...
        ..style = PaintingStyle.stroke
        ..strokeWidth = 1.0;
      canvas.drawLine(const Offset(0, 0), Offset(0, -leafSize * 0.95), veinPaint);

      canvas.restore();
    }

    Offset getPointOnStem(double t) {
      final double x = (1 - t) * (1 - t) * stemStart.dx + 2 * (1 - t) * t * stemControl.dx + t * t * stemEnd.dx;
      final double y = (1 - t) * (1 - t) * stemStart.dy + 2 * (1 - t) * t * stemControl.dy + t * t * stemEnd.dy;
      return Offset(x, y);
    }

    double getAngleOnStem(double t) {
      final Offset d = (stemControl - stemStart) * (2 * (1 - t)) + (stemEnd - stemControl) * (2 * t);
      return math.atan2(d.dy, d.dx);
    }

    drawLeaf(getPointOnStem(0.98), getAngleOnStem(0.98) - math.pi / 2.2, radius * 0.32);
    drawLeaf(getPointOnStem(0.90), getAngleOnStem(0.90) + math.pi / 2.1, radius * 0.28);
    drawLeaf(getPointOnStem(0.80), getAngleOnStem(0.80) - math.pi / 2.3, radius * 0.33);
    drawLeaf(getPointOnStem(0.68), getAngleOnStem(0.68) + math.pi / 1.9, radius * 0.31);
    drawLeaf(getPointOnStem(0.55), getAngleOnStem(0.55) - math.pi / 2.0, radius * 0.35);
    drawLeaf(getPointOnStem(0.40), getAngleOnStem(0.40) + math.pi / 2.2, radius * 0.32);
    drawLeaf(getPointOnStem(0.28), getAngleOnStem(0.28) - math.pi / 2.0, radius * 0.30);
    drawLeaf(getPointOnStem(0.15), getAngleOnStem(0.15) + math.pi / 2.3, radius * 0.25);
    ...
    void drawOlive(Offset pos, double size, double rotateRad) {
      ...
      final Paint oliveStemPaint = Paint()
        ...
        ..style = PaintingStyle.stroke
        ..strokeWidth = 1.5;
      canvas.drawLine(const Offset(0, 0), Offset(0, -size * 0.7), oliveStemPaint);

      canvas.translate(0, size * 0.4);

      final Rect oliveRect = Rect.fromCenter(center: const Offset(0, 0), width: size * 0.75, height: size * 1.15);
    ...
    drawOlive(olive1Pos + Offset(radius * 0.05, radius * 0.05), radius * 0.20, -0.15);

    final Offset olive2Pos = getPointOnStem(0.32);
    drawOlive(olive2Pos + Offset(-radius * 0.08, radius * 0.1), radius * 0.19, 0.25);
  }
